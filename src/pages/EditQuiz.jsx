...
const EditQuiz = () => {
  ...
  useEffect(() => {
    ...
    if (foundQuiz) {
      if (!isAuthenticated || foundQuiz.creator_id !== user?.id) {
        navigate('/');
        return;
      }
      ...
      setShowAnswerImmediately(foundQuiz.show_answer_immediately || false);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setError(`⚠️ Erro ao atualizar quiz: ${error.message}`);
      window.scrollTo({ top: 0, behavior: 'smooth' });
    } finally {
      setSaving(false);
    }
